-- =============================================================
-- BookMyCampus – Unified Database Schema
-- Merged from: schema.sql, bookings_schema.sql, timetable_schema.sql
-- Run this single file to set up the entire database from scratch.
-- =============================================================

-- ===================== IMPORTANT =====================
-- On shared hosting (e.g. InfinityFree), do NOT run CREATE DATABASE.
-- Instead, select your pre-created database in phpMyAdmin first,
-- then import this file via the "Import" tab.
-- =====================================================
...
CREATE TABLE IF NOT EXISTS notifications (
    ...
    target_role ENUM('admin', 'student', 'faculty', 'all') DEFAULT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    INDEX idx_user_id (user_id),
